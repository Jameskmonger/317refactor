...
//            Image image = Toolkit.getDefaultToolkit().getImage(signlink.findcachedir()+"mopar.jpg");
    ...
    private void blockCopy(int destinationPointer, int copyLength, int k, int sourceBlockLength, int sourcePointer, int destinationBlockLength,
    ...
    public void drawImage(int x, int y)
    {
        ...
        {
            blockCopyTransparent(DrawingArea.pixels, pixels, sourceOffset, destinationOffset, columnCount, rowCount, lineDestinationOffset, lineSourceOffset);
        }
    }

    private void blockCopyTransparent(int destination[], int source[], int sourcePointer, int destinationPointer, int copyLength, int i1,
    ...
    private void blockCopyAlpha(int sourcePointer, int blockCount, int destination[], int source[], int sourceBlockLength, int i1,
        ...
        for(int k2 = -i1; k2 < 0; k2++)
        {
            for(int ptr = -blockCount; ptr < 0; ptr++)
            {
                ...
                if(sourceValue != 0)
                {
                    ...
                    destination[destinationPointer++] = ((sourceValue & 0xff00ff) * alpha + (destinationValue & 0xff00ff) * destinationAlpha & 0xff00ff00) + ((sourceValue & 0xff00) * alpha + (destinationValue & 0xff00) * destinationAlpha & 0xff0000) >> 8;
                } else
                {
                    destinationPointer++;
                }
    ...
    public void rotate(int height, int rotation, int widthMap[], int hingeSize, int ai1[], int centreY,
                          int y, int x, int width, int centreX)
    {
        try
        {
            int negativeCentreX = -width / 2;
            int negativeCentreY = -height / 2;
            int offsetY = (int)(Math.sin((double)rotation / 326.11000000000001D) * 65536D);
            int offsetX = (int)(Math.cos((double)rotation / 326.11000000000001D) * 65536D);
