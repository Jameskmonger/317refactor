...
public final class GameFont extends DrawingArea {
	...
	/*
	 * Referred to as a 'glyph' rather than a
	 * 'character' as a character remains the same
	 * between fonts ('a' is always 'a') however
	 * a glyph is a representation of a character
	 * within a font.
	 * 
	 * http://graphicdesign.stackexchange.com/questions/13438/whats-the-practical-difference-between-a-glyph-and-a-character
	 */
	
	/*
	 * An array containing the pixels used
	 * in the glyph. Stored as either 1 or 0
	 * signifying whether that pixel should be
	 * drawn or not.
	 */
	...
	/*
	 * The spacing (kerning), horizontal and
	 * vertical, between glyphs in a font.
	 * 
	 ...
	 * 
	 * @param archive
	 * 			The archive containing the information about
	 * 			the font.
	 * 
	 * @param monospace
	...
	public GameFont(String name, Archive archive, boolean monospace) {
		...
		/*
		 * The buffer containing data about this
		 * specific font. The position of the font
		 * within the parent archive (index.dat)
		 * and the pixel data for the glyph.
		 */
		Buffer glyphData = new Buffer(archive.decompressFile(name + ".dat"));
		
		/*
		 * Stores the information about the glyphs,
		 * such as the kerning and the dimensons.
		 */
		Buffer glyphInformation = new Buffer(archive.decompressFile("index.dat"));
		
		/*
		 * Find the glyph information within the
		 * parent archive.
		 */
		glyphInformation.position = glyphData.getUnsignedLEShort() + 4;
		
		/*
		 * Find the glyph data for this font
		 * within the parent archive.
		 */
		...
		for (int g = 0; g < 256; g++) {
			...
			/*
			 * Set the pixels for the glyph based on
			 * whether it is square or rectangular.
			 */
			if (rectangular == 0) {
			...
			/*
			 * If the height of this glyph is higher than
			 * the highest glyph we've currently processed
			 * for this font, store this glyph's height as
			 * the highest.
			 * 
			 * 126 is the last visible character used in
			 * this client (~). No support for accented
			 * characters!
			 */
		...
		/*
		 * Character 32 is space, character 73 is uppercase I
		 * and character 105 is lowercase i.
		 */
		if (monospace) {
	...
	/**
	 * Renders an array of pixels into an output
	 * buffer with a specified colour.
	 * 
	 * @param input
	 * 			The array of pixel states (1 if drawn,
	 * 			0 if not drawn.)
	 * 
	 * @param inputPosition
	 * 			The position in the input array to start at.
	 * 
	 * @param inputWidth
	 * 			The amount to increase the position in the
	 * 			input array by after each iteration.
	 * 
	 * @param output
	 * 			The array to store the coloured pixels in.
	 * 
	 * @param outputPosition
	 * 			The position in the output array to start at.
	 * 
	 * @param outputWidth
	 * 			The amount to increase the position in the
	 * 			output array by after each iteration.
	 * 
	 * @param width
	 * 			The width of the group of pixels.
	 * 
	 * @param height
	 * 			The height of the group of pixels.
	 * 
	 * @param colour
	 * 			The colour to draw the pixels in.
	 */
	 ...
	 * 
	 * @param alpha
	 * 			The alpha value for the pixels.
	 */
	private void renderAlpha(byte input[], int inputPosition, int inputWidth, int output[], int outputPosition,
		 ...
		 */
		colour = ((colour & 0xff00ff) * alpha & 0xff00ff00)
				+ ((colour & 0xff00) * alpha & 0xff0000) >> 8;
		alpha = 256 - alpha;
	 ...
	 */
	private void drawGlyph(byte glyphPixels[], int x, int y, int width,
		 ...
		 */
		if (width > 0 && height > 0) {
			render(glyphPixels, inputPosition, inputWidth, DrawingArea.pixels, outputPosition,
					outputWidth, width, height, colour);
		}
	}

	private void drawGlyphAlpha(byte glyphPixels[], int x, int y, int width, int height,
		 ...
		 */
		if (width > 0 && height > 0)
			renderAlpha(glyphPixels, inputPosition, inputWidth, DrawingArea.pixels, outputPosition, outputWidth, width, height, colour, alpha);
	}
	
	/**
	 * Draws a string, including a shadow behind it if
	 * specified.
	 * 
	 * Used for interface drawing where the text could
	 * be shadowed but also could be shadowless.
	 * 
	 * @param text
	 * 			The text to draw.
	 * 
	 ...
	 * 
	 * @param shadowed
	 * 			Whether the text is shadowed or not.
	 */
	public void drawTextWithPotentialShadow(String text, int x, int y, int currentColour,
			boolean shadowed) {
		strikethrough = false;
		int originalX = x;
		if (text == null)
		...
		for (int c = 0; c < text.length(); c++) {
			 ...
			 * is a colour for (@dre@ for example)
			 */
			if (text.charAt(c) == '@' 
					&& c + 4 < text.length()
					&& text.charAt(c + 4) == '@') {
		...
		/*
		 * If the current piece of text has a strikethrough applied,
		 * draw a horizontal line from the start of the text to the
		 * current position.
		 */
		if (strikethrough) {
			DrawingArea.drawHorizontalLine(y + (int) (fontHeight * 0.7D),
					originalX, x - originalX, 0x800000);
		}
	}

	/**
	 * Draws centrally-aligned text, with a shadow
	 * if desired.
	 * 
	 * @param text
	 * 			The text to draw.
	 ...
	 * 
	 * @param shadow
	 * 			Whether or not a shadow should be drawn.
	 */
	public void drawCentredTextWithPotentialShadow(String text, int x, int y, int colour,
			boolean shadow) {
		drawTextWithPotentialShadow(text, x - getTextDisplayedWidth(text) / 2, y, colour,
				shadow);
	}

	/**
	 * Draws text with a random opacity over
	 * a shadow.
	 *
	 * @param text
	 * 			The text to draw.
	...
	public void drawAlphaTextWithShadow(String text, int x, int y, int currentColour,
		...
		for (int c = 0; c < text.length(); c++) {
			...
			if (text.charAt(c) == '@'
				...
				/*
				 * If the character is a space, we don't draw
				 * it - we just add the width of the space
				 * to the current x position.
				 */
				if (character != ' ') {
					drawGlyphAlpha(glyphPixels[character], x + horizontalKerning[character] + 1,
							y
									+ verticalKerning[character] + 1, glyphWidth[character], glyphHeight[character],
							0, 192);
					drawGlyphAlpha(glyphPixels[character], x + horizontalKerning[character],
							y
									+ verticalKerning[character], glyphWidth[character], glyphHeight[character],
							currentColour, alpha);
				}
				x += glyphDisplayWidth[character];
	...
	public void drawText(String text, int x, int y, int colour) {
		...
		for (int c = 0; c < text.length(); c++) {
			...
			if (character != ' ') {
				drawGlyph(glyphPixels[character], x
						+ horizontalKerning[character], y
						+ verticalKerning[character], glyphWidth[character],
						glyphHeight[character], colour);
			}
			x += glyphDisplayWidth[character];
		}
	}
	...
	public void drawCentredText(String text, int x, int y, int colour) {
		/*
		 * Draw the text with half of it to the
		 * left of the anchor, and half of it to
	...
	public void drawTextLeft(String text, int x, int y, int colour) {
		/*
		 * Draw text with all of the text to
		 * the left of an anchor point.
		 */
		drawText(text, x - getTextWidth(text), y, colour);
	}
	
	/**
	 * Draw text that is waving based on a given tick.
	 * The text is drawn around a central anchor point.
	 * The text follows a sine wave vertically.
	 * 
	 * @param text
	 * 			The text to draw.
	 * 
	 ...
	 * 
	 * @param tick
	 * 			The current tick (used to make the text wave).
	 */
	public void drawVerticalSineWaveText(String text, int x, int y, int colour,
			int tick) {
		if (text == null)
			return;
		
		/*
		 * Place half the text to the left of the
		 * anchor point and half the text to the
		 * right. This centres the text horizontally.
		 */
		x -= getTextWidth(text) / 2;
		
		/*
		 * Draw text from the top-left instead of
		 * the bottom.
		 */
		y -= fontHeight;
		...
		for (int c = 0; c < text.length(); c++) {
			char character = text.charAt(c);
			/*
			 * If the character is a space we do not draw it,
			 * but we still move the x position by the width
			 * of the space.
			 */
			if (character != ' ') {
				/*
				 * The y position of the text is determined using a
				 * sine wave.
				 */
				drawGlyph(glyphPixels[character],
						x + horizontalKerning[character],
						y + verticalKerning[character]
							+ (int) (Math.sin(c / 2D + tick / 5D) * 5D),
	...
	/**
	 * Draws text that shakes based on a given
	 * tick and how long the text has existed for.
	 * 
	 * @param text
	 * 			The text to draw.
	 * 
	 ...
	 * 
	 * @param elapsed
	 * 			The time the text has been drawn for.
	 * 
	 * @param tick
	 * 			The current tick (used to make the text wave).
	 */
	public void drawShakingText(String text, int x, int y, int colour,
		...
		for (int c = 0; c < text.length(); c++) {
			...
			if (character != ' ') {
				/*
				 * The vertical position of the text is decided based
				 * on a sine wave, taking into account the current
				 * time and the amount of time the text has been
				 * shaking for.
				 */
				drawGlyph(
						glyphPixels[character],
						x + horizontalKerning[character],
						y + verticalKerning[character]
							+ (int) (Math.sin(c / 1.5D + tick) * amplitude),
						glyphWidth[character], glyphHeight[character], colour);
			}
			x += glyphDisplayWidth[character];
		}
	 ...
	 * Draw text that is waving based on a given tick.
	 * The text is drawn around a central anchor point.
	 * The text follows a sine wave vertically and
	 * horizontally.
	 * 
	 * @param text
	 * 			The text to draw.
	 ...
	 * 			The current tick (used to make the text wave).
	 */
	public void drawVerticalHorizontalSineWaveText(String text, int x, int y, int colour,
			int tick) {
		if (text == null)
			return;
		...
		for (int c = 0; c < text.length(); c++) {
			...
			if (character != ' ') {
				/*
				 * The x and y positions of the text are determined
				 * using sine waves.
				 */
				drawGlyph(
						glyphPixels[character],
						x + horizontalKerning[character]
							+ (int) (Math.sin(c / 5D + tick / 5D) * 5D),
						y + verticalKerning[character]
							+ (int) (Math.sin(c / 3D + tick / 5D) * 5D),
						glyphWidth[character], glyphHeight[character], colour);
			}
			x += glyphDisplayWidth[character];
		}
	...
	/**
	 * Gets the numeric representation of a colour when
	 * given the RuneScape embedding value. Also handles
	 * strikethrough if applicable.
	 * 
	 * @param name
	 * 			The embed code of the effect.
	 * 
	 * @return The numeric representation of the colour,
	 * 			or -1 if the colour does not exist.
	 */
	...
	/**
	 * Get the displayed width of a text containing embedded
	 * colours, crowns etc.
	 * 
	 * @param text
	 * 			The string to get the width of.
	 * 
	 * @return The width of the string after the embedded
	...
	public int getTextWidth(String text) {
		...
		/*
		 * Iterate through every character in the text
		 * and add the displayed width of that character's
		 * glyph to the total width of the text.
		 */
		for (int c = 0; c < text.length(); c++) {
			width += glyphDisplayWidth[text.charAt(c)];
