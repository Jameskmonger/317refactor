...
public final class TextDrawingArea extends DrawingArea {
	...
	private void colourPixels(int pixels[], byte originalPixels[], int colour,
			int j, int k, int width, int height, int j1, int k1) {
		int _width = -(width >> 2);
		width = -(width & 3);
	...
	private void drawGlyph(byte glyphPixels[], int x, int y, int width,
		...
		if (!(width <= 0 || height <= 0)) {
			colourPixels(DrawingArea.pixels, glyphPixels, colour, i2, j1,
					width, height, k1, l1);
		}
	}

	private void drawShadowedChar(int i, int j, byte abyte0[], int k, int l,
			int i1, int j1) {
	...
	public void drawShadowedString(String text, int x, int y, int colour,
			boolean flag1) {
		strikethrough = false;
		int originalX = x;
		if (text == null)
		...
		for (int c = 0; c < text.length(); c++)
			if (text.charAt(c) == '@' && c + 4 < text.length()
					&& text.charAt(c + 4) == '@') {
		...
		if (strikethrough)
			DrawingArea.drawHorizontalLine(y
					+ (int) (charHeight * 0.69999999999999996D),
					originalX, x - originalX, 0x800000);
	}

	public void drawShadowedStringCentre(int i, int x, String text, int l,
			boolean flag) {
		drawShadowedString(text, x - getFormattedStringWidth(text) / 2, l, i,
				flag);
	}

	public void drawShadowedTextRight(String text, int x, int y, int colour,
			int seed) {
		if (text == null)
			return;
		random.setSeed(seed);
		...
		for (int c = 0; c < text.length(); c++)
			if (text.charAt(c) == '@' && c + 4 < text.length()
				...
				char character = text.charAt(c);
				if (character != ' ') {
					drawShadowedChar(192, x + horizontalKerning[character] + 1,
							glyphPixels[character], glyphWidth[character], y
									+ verticalKerning[character] + 1,
							glyphHeight[character], 0);
					drawShadowedChar(j1, x + horizontalKerning[character],
							glyphPixels[character], glyphWidth[character], y
									+ verticalKerning[character],
	...
	public void drawTextHRMidVTopWaving(String text, int x, int y, int colour,
			int wavePart) {
		if (text == null)
			return;
	...
	public void drawTextHRMidVTopWaving(String text, int x, int y, int colour,
			int wavePart, int wavePart2) {
		if (text == null)
			return;
		double d = 7D - wavePart / 8D;
		if (d < 0.0D)
			d = 0.0D;
		x -= getStringWidth(text) / 2;
		y -= charHeight;
	...
	public void drawTextHRMidVTopWaving2(String text, int x, int y, int colour,
			int wavePart) {
		if (text == null)
			return;
		x -= getStringWidth(text) / 2;
	...
	public int getFormattedStringWidth(String string) {
		...
		for (int c = 0; c < string.length(); c++)
			if (string.charAt(c) == '@' && c + 4 < string.length()
					&& string.charAt(c + 4) == '@')
				c += 4;
			else
	...
	private void recolourAlpha(byte abyte0[], int i, int j, int ai[], int l,
			int i1, int j1, int k1, int colour, int alpha) {
		...
		for (int j2 = -i; j2 < 0; j2++) {
			for (int k2 = -i1; k2 < 0; k2++)
				if (abyte0[l++] != 0) {
					int l2 = ai[j];
					ai[j++] = (((l2 & 0xff00ff) * alpha & 0xff00ff00)
							+ ((l2 & 0xff00) * alpha & 0xff0000) >> 8)
							+ colour;
				} else {
					j++;
				}
